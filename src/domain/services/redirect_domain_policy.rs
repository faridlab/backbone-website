//! Domain policy for Redirect aggregate
//!
...
/// Domain policy for Redirect — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Redirect>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type RedirectDomainPolicy = PermitAllPolicy<Redirect>;
