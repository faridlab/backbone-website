//! Domain policy for Visitor aggregate
//!
...
/// Domain policy for Visitor — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Visitor>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type VisitorDomainPolicy = PermitAllPolicy<Visitor>;
