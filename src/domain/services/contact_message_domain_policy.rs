//! Domain policy for ContactMessage aggregate
//!
...
/// Domain policy for ContactMessage — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<ContactMessage>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type ContactMessageDomainPolicy = PermitAllPolicy<ContactMessage>;
