//! Domain policy for WebsiteAuditLog aggregate
//!
...
/// Domain policy for WebsiteAuditLog — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<WebsiteAuditLog>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type WebsiteAuditLogDomainPolicy = PermitAllPolicy<WebsiteAuditLog>;
