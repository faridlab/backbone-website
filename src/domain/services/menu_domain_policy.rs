//! Domain policy for Menu aggregate
//!
...
/// Domain policy for Menu — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Menu>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type MenuDomainPolicy = PermitAllPolicy<Menu>;
